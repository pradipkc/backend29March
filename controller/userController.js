...
exports.register = async (req,res) => {
    // destructing to get info from user
...
exports.verifyUser = async (req, res) => {
    ...
    re.send({msg: "User verified successfully"})
}
//  to resend verification email
exports.resendVerification = async (req, res) => {
    //  check email
    let user = await User.findOne({email: req.boy.email})
    if(!user){
    ...
    token = await token.save()
    if(!token){
        return tes.status(400).json({error:"Failed to generate token"})
    }
    //  send email
...
exports.signIn = async (req, res) => {
    ...
    let token = jwt.sign({user:user._id, role: user.role}, process)
...
authentication - to identify user
    to authentication, we use email, password, username, phone number, etc

authentication - to allow user to access routes
...
*/
    ...
    exports.forgetPassword = async (req, res) => {
        // find email
        let user = await User.findOne({email: re.body.email})
        if(!user){
            return res.status(400).json({error:"User not registered"})
        }
        // generate token
